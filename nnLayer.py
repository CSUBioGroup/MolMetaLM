...
class Transformer_PostLN(nn.Module):
    ...
    def forward(self, input):
        x, maskPAD = input
        # x: batchSize × seqLen × feaSize; maskPAD: batchSize × seqLen × seqLen
        z = self.selfAttn(x, x, x, maskPAD) # => batchSize × seqLen × feaSize
        return (self.ffn(x, z),maskPAD) # => batchSize × seqLen × feaSize

class TransformerLayers_PostLN(nn.Module):
    def __init__(self, seqMaxLen, layersNum, feaSize, dk, multiNum, dropout=0.1, usePos=True, name='textTransformer'):
    ...
    def forward(self, x, maskPAD=None):
        ...
        if self.usePos:
            x = x+self.posEmb
        x = self.dropout(x) # => batchSize × seqLen × feaSize
        return self.transformerLayers((x, maskPAD)) # => batchSize × seqLen × feaSize
...
class TransformerLayers_Realformer(nn.Module):
    def __init__(self, layersNum, feaSize, dk, multiNum, maxRelativeDist=7, embDropout=0.1, hdnDropout=0.1, dkEnhance=1, 
